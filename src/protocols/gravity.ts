...
function gravityProtocol(
    request: ProtocolRequest,
    respond: (x: ProtocolResponse) => void
) {
    var cb = (
        statusCode: number,
        contentType: string | undefined,
        path: string,
        CSP: string | undefined = undefined
    ) => {
        const headers = {
    ...
    if (requestUrl == URIs.GRAVITY_NEW_USER) {
        ...
        cb(
            ...
            path.join(__dirname, "..", "ui", "global", "gravity-core.css")
        );
    } else if (requestUrl == URIs.GRAVITY_COMPONENTS) {
        cb(
            200,
            "application/javascript; charset=utf-8",
            path.join(__dirname, "..", "ui", "global", "components", "index.js")
        );
    } else if (requestUrl.startsWith(URIs.GRAVITY_NEW_USER)) {
        serveAppAsset(
            requestUrl,
            path.join(__dirname, "..", "ui", "user-land", "new-user"),
            cb
        );
        ...
        serveAppAsset(
            requestUrl,
            path.join(__dirname, "..", "ui", "browser"),
            cb
        );
    } else if (requestUrl.startsWith(URIs.GRAVITY_MODALS)) {
        serveAppAsset(
            requestUrl,
...
async function serveAppAsset(
    ...
    { CSP }: { CSP: string | undefined } = { CSP: undefined }
) {
